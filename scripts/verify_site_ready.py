...
⚠️ **對正式站要連著 Sam 的 VPN 才跑得動**（2026-08-25 起）。維運移除 miniOrange
的同時，在 WAF 加了 IP 白名單。實測：VPN 出口 `98.45.146.189` 在名單內（HTTP 200），
不連 VPN 的一般網路會被 challenge 擋掉（HTTP 202），停在檢查 1。

看到 202 就是這個原因，不是站台壞了。要不連 VPN 也能看正式站狀態的話，
從 n8n 跑 `n8n/diagnose-prod-access.workflow.json`（唯讀）。
測試站不受白名單影響。
...
def diagnose(code, payload):
    ...
    if waf:
        return (f"AWS WAF 的 {waf} 動作（HTTP {code}）。"
                ...
                f"       2026-08-25 起正式站的 API 有 IP 白名單。"
                f"**先確認 VPN 連著**——實測 VPN 出口在名單內。\n"
                f"       n8n 也在名單內，流程不受影響；被擋的只有本機。\n"
