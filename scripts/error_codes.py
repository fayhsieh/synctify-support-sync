...
CODES = [
    ...
    # 2026-09-16 測試站 6627 上傳圖片時實際收到 504（站台前面的 nginx 等不到回應）。
    # 跟 B2 分開：B2 是連不上，這裡是連上了、站台自己來不及回。
    ("B3", ["gateway time-out", "gateway timeout", "bad gateway", "service unavailable"],
     "站台前面的閘道等不到回應就先斷了（504／502／503），多半是圖片多或站台當下太慢。",
     "等一兩分鐘再按一次同步。重按是更新同一篇、不會多一篇文章，"
     "已經上傳成功的圖片會沿用同名檔案、不會重複進媒體庫。"
     "同一篇連續三次都卡在同一步，請找 Fay。"),
