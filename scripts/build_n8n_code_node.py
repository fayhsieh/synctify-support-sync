...
TP_OUT = ROOT / "n8n" / "tp-code-node.py"     # Workflow 3 的區塊抽取節點
...
def build_tp():
    """Workflow 3 的區塊抽取 Code node。"""
    src = (CONVERTER / "tp_blocks.py").read_text(encoding="utf-8")
...
def main():
    ...
    tp_body = build_tp()
    TP_OUT.write_text(tp_body, encoding="utf-8")
    print(f"✓ 已產生 {TP_OUT}（{len(tp_body.splitlines())} 行）")
