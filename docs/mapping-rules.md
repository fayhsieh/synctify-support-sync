...
**⚠️ alt 屬性必須編碼 `>`**（2026-09-01 正式站 5620 實際踩到）。WordPress 核心的
`img_caption_shortcode()` 用 `#((?:<a [^>]+>\s*)?<img [^>]+>(?:\s*</a>)?)(.*)#is`
抓圖片與圖說，`<img [^>]+>` 的 `[^>]+` **在第一個 `>` 就停住**——alt 裡有 `>` 的話
img 標籤會被從中切斷，後半段（`… width="1024" height="605" />`）直接印在頁面上。
UI 路徑用 `>` 分隔是寫作慣例，所以這不是罕見情況。由 `esc_attr()` 統一編碼
`& < > "`；圖說本身是文字內容，不編碼。
...
**留言節點必須直接接在原因節點後面**——`$json` 指的是「上一個節點的輸出」，
中間隔著回寫（HTTP 節點）的話，留言看到的是 Notion PATCH 的回應，裡面沒有
